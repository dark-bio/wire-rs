...
use crate::transport::{self, Emitter};
...
/// Handler of the session ending without a close, with the reason.
pub(super) type Disconnect = Box<dyn FnOnce(Error) + Send>;
...
impl<Out: Envelope, In: Envelope> Switchboard<Out, In> {
    /// Starts the switchboard of the side allocating ids of the parity over a
    /// source, the reader and the worker threads with it, the closer ending
    /// the transport when the multiplexer closes or the session fails.
    pub(super) fn start(parity: Parity, source: impl Source, closer: Closer) -> Arc<Self> {
    ...
    /// Sends an encoded message through the session's emitter, ending the
    /// multiplexer if the failure took the session with it. A message refused
    /// before sealing leaves the session alone.
    pub(super) fn send(&self, message: &[u8]) -> Result<(), Error> {
        ...
        match self.emitter().send_message(message) {
            ...
            // The first failure names the reason, a send refused because the
            // session ended meanwhile surfaces that one instead of its own
            Err(err) => {
    ...
    /// nor error, is the peer breaking the protocol and ends the session.
    fn route(&self, message: Vec<u8>) {
    ...
    /// peer filling the inbox past its limit overran its window, which ends
    /// the session rather than serving it.
    fn push(&self, request: Request<In>) {
        let mut inbox = lock(&self.inbox);
        if inbox.bytes + request.bytes > INBOX {
            drop(inbox);
            warn!("ending session, peer overran its window");
            self.fail(Error::Flooded);
            return;
        }
    ...
    fn fail(&self, reason: Error) {
        if self.end(State::Failed(reason.clone()))
            && let Some(handler) = lock(&self.disconnect).take()
        {
            handler(reason);
        }
    }
    ...
    fn end(&self, state: State) -> bool {
        {
            ...
            registry.inflight = 0;
            for (_, (tx, _)) in std::mem::take(&mut registry.pending) {
                let _ = tx.send(Err(registry.refusal()));
            }
        }
        self.room.notify_all();
        {
...
/// Reads messages until the session ends, routing each, the end failing the
/// multiplexer with the reason, a close needing no notification. The source
/// goes with the thread, ending the session for the emitters.
fn read<Out: Envelope, In: Envelope>(
    ...
    loop {
        match source.next_message() {
            Ok(message) => switchboard.route(message),
            Err(err) => {
